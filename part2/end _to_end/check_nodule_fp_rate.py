...
from part2.segmentation.dsets import Luna2dSegmentationDataset, getCt, getCandidateInfoList, getCandidateInfoDict, \
    CandidateInfoTuple
from part2.util.util import xyz2irc, irc2xyz
from part2.segmentation.model import UNetWrapper
import logging
            ...
            for batch_ndx, batch_tup in enumerate(cls_dl):
                ...
                for i, _series_uid in enumerate(series_list):
                    assert series_uid == _series_uid, repr(
                        [batch_ndx, i, series_uid, _series_uid, seg_candidateInfo_list])
    ...
    def clusterSegmentationOutput(self, series_uid, ct, clean_g):
        ...
        for i, center_irc in enumerate(centerIrc_list):
            assert np.isfinite(center_irc).all(), repr(
                [series_uid, i, candidate_count, (ct.hu_a[candidateLabel_a == i + 1]).sum(), center_irc])
            center_xyz = irc2xyz(
                center_irc,
                ct.origin_xyz,
                ct.vxSize_xyz,
